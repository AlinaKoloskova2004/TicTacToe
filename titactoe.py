...
class TicTacToe:
    ...
    def __init__(self, player_x, player_0):  # конструктор
        ...
        self.reload() #перезагрузка

    def move(self):  # ход
        def is_move_valid(x, y): #функция, на действительность шага
            return 1 <= x <= 3 and 1 <= y <= 3 and self.table[x-1][y-1] == '*'

        print(f'Ход игрока {self.get_current_player_name()}:')
        is_coords_valid = False
        x, y = 0, 0
        while not is_coords_valid: #цикл на правильность координат
            y, x = input('Введите координаты x и y через пробел: ').split() #Разбиение строки на подстроки
            x, y = int(x), int(y)
            is_coords_valid = is_move_valid(x, y)
            if not is_coords_valid: #проверка на координаты, т.е, если координаты введены не правильно, то будет выведена ошибка
    ...
    def get_winner(self):  # поиск победителя
        count_stars = 0 #счётчик очков
        for line in self.table:
            if line[0] != '*' and len(set(line)) == 1:
                self.winner = self.current_move #текущий ход победителя
                return None
            if '*' in line:
                count_stars += 1
    ...
    def reload(self):  # перезагрузка игры
        print('Началась игра!')
        self.table = [['*', '*', '*'], ['*', '*', '*'], ['*', '*', '*']] #создание таблицы
        if self.winner == '' or self.winner == 'n':
            self.current_move = 'x' #текущий ход крестика
        else:
    ...
    def start(self):  # начало игры
        while True:
            ...
            if self.winner:  # если победитель есть - завершаем игру
                break
            self.change_current_move() #изменить текущий ход
    ...
    def get_current_player_name(self):  # получение имени текущего игрока
        return self.player_x if self.current_move == 'x' else self.player_0 #возврат хода игрока

    def __str__(self):  # преобразование объекта к строке
        result = ""
        for line in self.table: #создание таблицы
